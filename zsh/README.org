...
function backup() {
    if [ -z "$1" ]; then
        ...
        sudo rsync -Pa -e "ssh -p 2222 -i $HOME/.ssh/tbcom_ssh" redacted@redacted:/home/www/redacted "$1"
        true
    fi
}
#+end_src
...
|---------+--------------------------+-----------------------------------------------------|
| =b=       | N/A                      | Show ANSI weather in Denver today                   |
...
#+begin_src sh :tangle .zshrc
alias b="ansiweather -l \"Denver\" -u imperial -H true -h false -p false -i false -s true"
